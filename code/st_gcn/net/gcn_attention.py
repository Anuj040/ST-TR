...
class gcn_unit_attention(nn.Module):
    def __init__(
        ...
        self.incidence = Variable(self.incidence.clone(), requires_grad=False).view(
            -1, self.incidence.size()[-1], self.incidence.size()[-1]
        )
        ...
        if not self.only_attention:
            self.g_convolutions = nn.ModuleList(
                [
                    ...
                    for _ in range(self.incidence.size()[0])
                ]
            )

            self.attention_conv = spatial_attention(
    ...
    def forward(self, x, label, name):
        ...
        # Adjacence matrix
        self.incidence = self.incidence.to(x.get_device())
        ...
        if not self.only_attention:
            ...
            for i, partition in enumerate(incidence):
                ...
                y = (
                    self.g_convolutions[i](xp)
                    if i == 0
                    else y + self.g_convolutions[i](xp)
                )

            # Concatenate on the channel dimension the two convolutions
            y = torch.cat((y, attn_out), dim=1)
        elif self.skip_conn and self.in_channels == self.out_channels:
            y = attn_out + x_sum
        else:
            y = attn_out
        if self.bn_flag:
