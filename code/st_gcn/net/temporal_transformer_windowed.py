...
class TcnUnitAttentionBlock(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        if self.relative:
            if self.more_channels:
                self.key_rel = nn.Parameter(
                    torch.randn(
                        (2 * self.block_dim - 1, self.dk // self.num),
                        requires_grad=True,
                    )
                )
                ...
                self.key_rel = nn.Parameter(
                    torch.randn(
                        (2 * self.block_dim - 1, self.dk // Nh), requires_grad=True
                    )
                )
    ...
    def forward(self, x):
        ...
        for i in range(T1 // self.block_dim):

            block = x[
                :, :, :, i * self.block_dim : (i * self.block_dim + self.block_dim)
            ]
            N, C, _, T = block.shape
            flat_q, flat_k, flat_v, q, k, v = self.compute_flat_qkv(
                block, self.dk, self.dv, self.Nh
            )
            ...
            if self.drop_connect and self.training:
                mask = torch.bernoulli((0.5) * torch.ones(B * self.Nh * T))
            ...
            if not self.more_channels:
                attn_out = torch.reshape(
                    attn_out, (B, self.Nh, 1, T, self.dv // self.Nh)
                )
            else:
                attn_out = torch.reshape(
                    attn_out, (B, self.Nh, 1, T, self.dv // self.num)
                )
